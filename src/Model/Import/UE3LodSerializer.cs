...
{
    public SerializedLodModel SerializeLodModel(FStaticLODModel lod, MeshImportContext context)
    {
    ...
    public byte[] SerializeSkeletalMeshBody(
        USkeletalMesh skeletalMesh,
        IReadOnlyList<FStaticLODModel> lodModels,
        FStaticLODModel sourceLodOverride,
        MeshImportContext context)
    {
        using UE3BinaryWriter writer = new();

        WriteBounds(writer, skeletalMesh.Bounds);
        WriteObjectArray(writer, skeletalMesh.Materials);
        WriteVector(writer, skeletalMesh.Origin);
        WriteRotator(writer, skeletalMesh.RotOrigin);
        WriteArray(writer, skeletalMesh.RefSkeleton, WriteMeshBone);
        writer.Write(skeletalMesh.SkeletalDepth);

        writer.Write(lodModels.Count);
        foreach (FStaticLODModel lod in lodModels)
            writer.WriteBytes(SerializeLodModel(lod, context).Bytes);

        WriteNameIndexMap(writer, skeletalMesh.NameIndexMap);
        WriteArray(writer, skeletalMesh.PerPolyBoneKDOPs, WritePerPolyBoneCollision);
        WriteStringArray(writer, skeletalMesh.BoneBreakNames);
        WriteByteBlob(writer, skeletalMesh.BoneBreakOptions);
        WriteObjectArray(writer, skeletalMesh.ClothingAssets);
        WriteArray(writer, skeletalMesh.CachedStreamingTextureFactors, static (w, value) => w.Write(value));

        writer.Write(skeletalMesh.SourceData.bHaveSourceData);
        if (skeletalMesh.SourceData.bHaveSourceData)
            writer.WriteBytes(SerializeLodModel(sourceLodOverride ?? skeletalMesh.SourceData.LODModel, context).Bytes);
    ...
    private static void WriteBounds(UE3BinaryWriter writer, FBoxSphereBounds value)
    {
        WriteVector(writer, value.Origin);
        WriteVector(writer, value.BoxExtent);
        writer.Write(value.SphereRadius);
    }

    private static void WriteMeshBone(UE3BinaryWriter writer, FMeshBone value)
    {
        WriteName(writer, value.Name);
        writer.Write(value.Flags);
        WriteQuat(writer, value.BonePos.Orientation);
        WriteVector(writer, value.BonePos.Position);
        writer.Write(value.NumChildren);
        writer.Write(value.ParentIndex);
        WriteColor(writer, value.BoneColor);
    }

    private static void WritePerPolyBoneCollision(UE3BinaryWriter writer, FPerPolyBoneCollisionData value)
    {
        WriteUnknownElementArray(writer, value.LegacykDOPTree.Nodes);
        WriteUnknownElementArray(writer, value.LegacykDOPTree.Triangles);
        WriteArray(writer, value.CollisionVerts, WriteVector);
    }

    private static void WriteUnknownElementArray(UE3BinaryWriter writer, UArray<byte[]> values)
    {
        int elementSize = values.Count == 0 ? 0 : values[0].Length;
    ...
    private static void WriteNameIndexMap(UE3BinaryWriter writer, UMap<FName, int> map)
    {
        writer.Write(map.Count);
        foreach ((FName key, int value) in map)
        {
    ...
    private static void WriteObjectArray(UE3BinaryWriter writer, UArray<FObject> values)
    {
        writer.Write(values.Count);
        foreach (FObject value in values)
            writer.Write(value?.TableEntry?.TableIndex ?? 0);
    }

    private static void WriteStringArray(UE3BinaryWriter writer, UArray<string> values)
    {
        writer.Write(values.Count);
    ...
    private static void WriteName(UE3BinaryWriter writer, FName value)
    {
        writer.Write(value.Index);
        writer.Write(value.Numeric);
    }
    ...
    private static void WriteQuat(UE3BinaryWriter writer, FQuat value)
    {
        writer.Write(value.X);
        writer.Write(value.Y);
        writer.Write(value.Z);
    ...
    private static void WriteRotator(UE3BinaryWriter writer, FRotator value)
    {
    ...
    private static void WriteUnrealString(UE3BinaryWriter writer, string value)
    {
        value ??= string.Empty;
        bool unicode = value.Any(static c => c > 255);
        if (unicode)
        {
