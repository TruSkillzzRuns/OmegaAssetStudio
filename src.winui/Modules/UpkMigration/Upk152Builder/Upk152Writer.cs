...
public sealed class Upk152Writer
{
    ...
    public async Task<MigrationResult> WriteAsync(MigrationPackagePlan plan, Action<string>? log = null)
    {
        ...
        PrepareHeader(plan.SourceDocument.RawHeader, plan.Header, outputPath);
        log?.Invoke($"Writing new UPK package: {outputPath}");
        ...
        try
        {
            ...
            plan.SourceDocument.RawHeader.FullFilename = tempOutputPath;
            await repository.SaveUpkFile(plan.SourceDocument.RawHeader, tempOutputPath, log).ConfigureAwait(false);
